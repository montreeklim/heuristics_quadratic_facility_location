...
clone the repository as usual (with `git clone https://github.com/Malena205/heuristics_quadratic_facility_location.git`) to get the actual data files. Then, navigate into the folder as usual (with `cd`) and pull the large data files with Git LFS (`git lfs pull`)
